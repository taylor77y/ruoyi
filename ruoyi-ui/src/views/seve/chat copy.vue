    ...
    <el-row>
      <div class="titleBox">
        ...
        <div v-if="act !== null">
          ...
          <span class="head-title">账户类型: {{oneList.useType == 1?'正式用户':'测试用户'}}</span>
          <span class="head-title">推荐人: {{oneList.welMemberPhone}}</span>
          <span class="head-title">登录IP: {{ oneList ? oneList.lastLoginIp: '' }} </span>
      ...
      <div class="userList">
        ...
        <div class="userListBox">
          <div v-for="(item, index) in userListData" :key="index" @click="getAct(item, index)"
            ...
            <el-badge :is-dot="[null, '3'].includes(item.session.lastId) || receiver === item.user.id ? false:true" class="item">
            ...
            <div v-if="sender !== item.source">
              ...
              <div class="word">
                ...
                <div class="info">
                  ...
                  <div class="info-content" v-if="item.type === 1">{{ item.body }}</div>
  ...
  export default {
    data() {
      return {
        ...
        socket: new socket(`wss://qcim.m1ob.com/service/message/` + getId()),
        // 在线状态
        state: 1,
        //搜索用户
        search: "",
