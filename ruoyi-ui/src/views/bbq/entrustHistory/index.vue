<template>
  <div class="app-container">
    <el-form :model="queryParams" ref="queryForm" size="small" :inline="true" v-show="showSearch" label-width="108px">
      ...
      <el-form-item label="撮合方ID" prop="matchMember">
        <el-input
          v-model="queryParams.matchMember"
          placeholder="请输入撮合方ID"
          clearable
          @keyup.enter.native="handleQuery"
        />
      </el-form-item>
    ...
    <el-row :gutter="10" class="mb8">
      <!-- <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleAdd"
          v-hasPermi="['system:entrustHistory:add']"
        >新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="single"
          @click="handleUpdate"
          v-hasPermi="['system:entrustHistory:edit']"
        >修改</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['system:entrustHistory:remove']"
        >删除</el-button>
      </el-col> -->
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleExport"
          v-hasPermi="['system:entrustHistory:export']"
        >导出</el-button>
      </el-col>
      <right-toolbar :showSearch.sync="showSearch" @queryTable="getList"></right-toolbar>
    </el-row>

    <el-table v-loading="loading" :data="entrustHistoryList" @selection-change="handleSelectionChange">
      ...
      <!-- <el-table-column label="${comment}" align="center" prop="priceType" /> -->
      <el-table-column label="状态" align="center" prop="state" />
      <el-table-column label="手续费" align="center" prop="tradeFee" />
      <el-table-column label="撮合方" align="center" prop="matchPhone" />
    ...
    <el-dialog :title="title" :visible.sync="open" width="500px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        ...
        <el-form-item label="撮合方ID" prop="matchMember">
          <el-input v-model="form.matchMember" placeholder="请输入撮合方ID" />
        </el-form-item>
        <el-form-item label="撮合总额" prop="matchFee">
...
<script>
import { listEntrustHistory, getEntrustHistory, delEntrustHistory, addEntrustHistory, updateEntrustHistory } from "@/api/system/entrustHistory";
...
export default {
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      listEntrustHistory(this.queryParams).then(response => {
        this.entrustHistoryList = response.rows;
    ...
    handleUpdate(row) {
      ...
      getEntrustHistory(id).then(response => {
      ...
      this.$refs["form"].validate(valid => {
        if (valid) {
          if (this.form.id != null) {
            updateEntrustHistory(this.form).then(response => {
              this.$modal.msgSuccess("修改成功");
              this.open = false;
              this.getList();
            });
          } else {
            addEntrustHistory(this.form).then(response => {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除历史委托记录编号为"' + ids + '"的数据项？').then(function() {
        return delEntrustHistory(ids);
      }).then(() => {
        this.getList();
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('system/entrustHistory/export', {
        ...this.queryParams
      }, `entrustHistory_${new Date().getTime()}.xlsx`)
    }
  }
};
</script>
